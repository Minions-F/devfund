...
public class Squarelotron {
    ...
    public Squarelotron(int n) {
        ...
        for (int fil = 0; fil < n; fil++) {
            for (int col = 0; col < n; col++) {
                this.squarelotron[fil][col] = numbers;
                numbers++;
            }
        }
    }
        ...
        for (int fil = 0; fil < size; fil++) {
            for (int col = 0; col < size; col++) {
                if (numbersInRing(fil, col, ring)) {
                    newSquarelotron.squarelotron[fil][col] = this.squarelotron[size - 1 - fil][col];
                }
            }
        }
    ...
    public Squarelotron mainDiagonalFlip(int ring) {
        ...
        for (int fil = 0; fil < size; fil++) {
            for (int col = 0; col < size; col++) {
                if (numbersInRing(fil, col, ring)) {
                    newSquarelotron.squarelotron[fil][col] = this.squarelotron[col][fil];
                }
            }
        }
        return newSquarelotron;
    ...
    public void rotateRight(int numberOfTurns) {
        ...
        for (int n = 0; n < numberOfTurns; n++) {
            ...
            for (int fil = 0; fil < size; fil++) {
                for (int col = 0; col < size; col++) {
                    newSquarelotron.squarelotron[col][size - 1 - fil] = squarelotron[fil][col];
                }
            }
